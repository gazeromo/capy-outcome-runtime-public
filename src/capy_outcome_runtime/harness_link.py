...
class HarnessLink:
    ...
    def connection_contracts(self, value, secret, peer):
        fields = 'site_id device_id client_id'
        closed(value, fields + (' contract' if isinstance(value, dict) and 'contract' in value else ''))
        token(value['client_id'], r'cli_[0-9a-f]{32}')
        if 'contract' in value:
            token(value['contract'], r'[a-z][a-z0-9.]{0,79}/v[1-9][0-9]{0,2}')
        proof = self.link.proof(secret, peer)
        with self.link.access.authority_transition_guard(), self.link.db() as db:
            device = self._device(db, value, proof)
            self._scope(db, device)
            client = db.execute('SELECT checked FROM harness_clients WHERE device=? AND client=?',
                                (device['id'], value['client_id'])).fetchone()
            if client is None or client['checked'] is None:
                raise LinkError('CLIENT_CHECK_REQUIRED', 409)
            from .managed_connection_contracts import contracts
            return contracts(value.get('contract'))
