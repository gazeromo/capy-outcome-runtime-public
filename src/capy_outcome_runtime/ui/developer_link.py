...
def render_development_section(statuses):
    return _safe('<section aria-label="Local development"><h2>Develop on this computer</h2><a class="button" href="/developer">Build an app</a>' + ''.join(card(s) for s in statuses) + '</section>')


def render_setup(auth, devices, statuses, site_origin, site_id, now, *, transfer_enabled=False, clients=()):
    content = introduction()
    content += '<h2>Connect this computer</h2><p>This desktop preview requires macOS, Python 3.11 or newer, native Git, and an installed Codex desktop client. Sign in to Codex with your own account. Apple Command Line Tools, including <code>swiftc</code>, are required to register the macOS URL handler. Complete normal project trust prompts when Codex asks.</p>'
    version = '0.6.0' if transfer_enabled else '0.5.0'
    content += '<p>Install the exact Capy Developer '+version+' wheel supplied with this preview handoff. In the folder containing that wheel, run:</p><pre><code>python3 -m pip install ./capy_developer-'+version+'-py3-none-any.whl\ncapy-dev doctor --json</code></pre>'
    content += '<p><a href="https://github.com/gazeromo/capy-developer/blob/9f0d0334e1eedc230f109d13bc9893aac0d56750/README.md">Developer installation background</a>. Use the supplied preview wheel for this connection; the historical source instructions do not include this desktop bridge. Windows and Linux desktop handlers are not qualified by this preview. This does not add a macOS application runtime or acceptor.</p>'
    content += '<p>Then run this one-time setup command:</p><pre><code>' + esc('capy-dev setup --site ' + site_origin + ' --site-id ' + site_id) + '</code></pre><p>Compare the named computer and confirmation code shown by setup before approving. The connection expires after 90 days. Revocation stops new reports; local code remains on your computer.</p>'
    for dev in devices:
        ...
        if not dev['revoked'] and dev['expires'] > now:
            ...
            content += form(auth, '/developer/requests', 'Build an app', device_id=dev['id'], idempotency_key=secrets.token_hex(16))
...
def render_request(auth, site_id, status, path, handoff):
    ...
    content += '<p><a href="/developer">Manage connected computers</a></p>'
    return _safe(content)
