...
def route(handler, method, parsed):
    ...
    try:
        ...
        if parsed.path.startswith('/api/'):
            ...
            elif (match := re.fullmatch(r'/api/developer-link/harness-v0/(status|register|check|begin|reopen|connection-contracts)', parsed.path)):
                result = getattr(HarnessLink(link), match[1].replace('-', '_'))(value, secret, peer)
            else:
        ...
        if parsed.path in ('/developer', '/developer/overview') and method == 'GET':
            ...
            content = render_setup(auth, devices, statuses, link.origin, link.site_id, link.clock(),
                                   ...
                                   clients=rows, conversations=handler.product.chat_store.list_conversations(auth.actor)[:30])
            ...
            if method == 'POST' and match[2] == 'approve':
                ...
                handler.redirect('/developer/overview')
                return True
            ...
            if conversation:
                with handler.product.chat_store.connect() as db:
                    handler.product.chat_store._conversation(db,auth.actor,conversation)
            req = link.create(auth.actor, fields.get('device_id',''), fields.get('idempotency_key',''))
            if conversation:
                from .work_context import linked_conversation
                linked_conversation(handler.product,auth.actor,req['handoff_id'],conversation)
            ...
            if method == 'POST' and action:
                if action == 'continue':
                    ...
                    from .work_context import linked_conversation
                    original = linked_conversation(handler.product,auth.actor,handoff)
                    if original:linked_conversation(handler.product,auth.actor,req['handoff_id'],original['id'])
            ...
            if original:
                from .ui_compatibility import compatibility_body
                content = compatibility_body('<p><a href="'+esc(original['href'])+'">Return to '+esc(original['title'])+'</a></p><p>This app change is separate from the original business request, which remains saved.</p>'+str(content))
            if getattr(handler.product, "release_workflow", None) is not None and (status.get("snapshot") or {}).get("candidate_id"):
                from .ui_compatibility import compatibility_body
                content = compatibility_body(str(content).replace('</div><a href=', '<a class="button" href="/releases/review/'+handoff+'">Review for testing</a></div><a href=', 1))
        ...
        if parsed.path.startswith('/api/'):
            ...
            handler.send_html('<h1>Developer link unavailable</h1><p>' + esc(str(exc) if isinstance(exc, LinkError) else 'Current authorization or request is invalid.') + '</p><a href="/developer/overview">Return to development</a>', status, referrer_policy="same-origin")
    return True
