    ...
    def add(self, actor, preview_id, membership_id, idempotency_key):
        ...
        with access.guarded_actor(actor), self.previews._lock:
            ...
            with self.previews.context(actor, preview_id) as (_row, mapped, interface):
                outcome=interface.activity(mapped, preview['record']['application_id'], preview['successful_activity'])
                if outcome.get('result',{}).get('status')=='needs_input':fail('RELEASE_PREVIEW_REQUIRED')
            ...
            with store.transaction() as db:
                ...
                if target.workspace_kind == 'team':
                    ...
                    connections = self.previews.bindings(
                        descriptor, record['version_digest'], target.team_id,
                        target.execution_scope_id, target.membership_id) if descriptor.connections else {}
                    store.bind(target.execution_scope_id, record['application_id'], record['version_digest'], connections)
    ...
    def remove(self, actor, application_id):
        with self.workflow.access.guarded_actor(actor), self.workflow.store.transaction() as db:
            ...
            if actor.workspace_kind == 'team':
                ...
                binding = self.workflow.store.binding_or_none(actor.execution_scope_id, application_id)
                if binding is not None and binding.connections and self.previews.connection_control is not None:
                    self.previews.connection_control.revoke_bindings(binding.connections)
